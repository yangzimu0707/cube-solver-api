...
def solve_555(state_str):
    ...
    # 快速校验：颜色数量不对说明不是合法的 3x3 状态，无需调用 Kociemba
    if not is_valid_3x3(cube_string):
        raise ValueError("魔方尚未完成降阶（中心/棱块未对齐），无法在线求解")

    # Solve with Kociemba（子进程 + 超时保护，防止不可解状态拖垮服务器）
    try:
        solution_3x3 = solve_with_timeout(cube_string)
