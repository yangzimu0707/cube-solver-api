...
class CenterLibrary:
    ...
    def build(self, conj_depth=None):
        ...
        # 4x4 补充宽层环（TR/TL/TU/TD）：
        # L<->R 面中心贴纸只有经 MU/MD 类 y 层转动才能交换，纯 slice 环对易子
        # 无法织出这类 3-cycle（实验验证 (L,R) 有序对全部缺失，导致 R 面最后一个
        # 槽从 L 面取块时无原语可用）。宽层环 + 共轭深度 2 后 (p,s) 覆盖完整。
        wides = ["TR", "TL", "TU", "TD"] if n == 4 else []

        # 1. 所有环 slice face slice'（4x4 另有宽层环）
        rings = {}
        for s in slices:
            for f in FACES:
        ...
        for (s1, f1), p in items:
            for (s2, f2), q in items:
                ...
                for comm in (p + q + p_inv + q_inv, p + q_inv + p_inv + q):
                    cyc = _cycle_decomp(comm, n, slot_names)
                    if len(cyc) == 1 and len(cyc[0]) == 3:
                        c = cyc[0]
                        base_moves.setdefault(tuple(c), comm)
                        base_moves.setdefault(tuple(c[::-1]), [_inv(m) for m in reversed(comm)])

        # 3. 迭代共轭：单个 3-cycle 的共轭仍是单个 3-cycle（中心槽置换），
        #    无需逐条立方体验证。共轭轮数 5x5 需要更多，4x4 因宽层环需 2 轮。
        if conj_depth is None:
            conj_depth = 3 if n >= 5 else (2 if n == 4 else 1)
        conj_algs = []
        for f in FACES:
            conj_algs += [f, f + "2", f + "'"]
        for s in slices:
        ...
        self._by_sorted = {}
        self._by_pair = {}  # (p, s) -> [(cyc, moves)]：把 p 位置的块放入 s 的候选原语
    ...
    def _add(self, cyc, moves):
        ...
        for i in range(3):
            p, s = cyc[i], cyc[(i + 1) % 3]
            self._by_pair.setdefault((p, s), []).append((tuple(cyc), moves))

    def find_cycle(self, slots, must_include, free_rule):
        """寻找一个 3-cycle：把 slots[1] 位置的块放入 slots[0]。
        要求第三个槽满足 free_rule。
        优先最短原语（惰性排序一次）。返回 (oriented_cycle, moves) 或 None。
        """
        s, p = slots
        pairs = getattr(self, "_by_pair", None)
        if pairs is not None:
            # 惰性排序：各 (p,s) 候选按动作长度升序，插入优先选最短原语
            if not getattr(self, "_by_pair_sorted", False):
                for lst in pairs.values():
                    lst.sort(key=lambda e: len(e[1]))
                self._by_pair_sorted = True
            iterable = pairs.get((p, s), ())
        else:
            # 旧版缓存（无 _by_pair 索引）回退：全表扫描
            iterable = [
                (cyc, moves)
                for key, entries in self._by_sorted.items()
                if s in key and p in key
                for cyc, moves in entries
                if (cyc[0] == p and cyc[1] == s) or
                   (cyc[1] == p and cyc[2] == s) or
                   (cyc[2] == p and cyc[0] == s)
            ]
        for cyc, moves in iterable:
