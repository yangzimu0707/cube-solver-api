...
# ---------- 步数优化辅助（快速路径 / 动作化简 / 最短原语） ----------

def _move_base(m):
    """去掉 ' / 2 后缀，得到基底动作名（如 TR' -> TR）"""
    if m.endswith("'") or m.endswith("2"):
        return m[:-1]
    return m


def _move_exp(m):
    """动作的幂指数：X=1, X2=2, X'=3（mod 4 下 X' 等价 X³）"""
    if m.endswith("'"):
        return 3
    if m.endswith("2"):
        return 2
...
def _is_inverse(a, b):
    """a、b 是同一基底且互为逆（或同为 X2）。"""
    return _move_base(a) == _move_base(b) and (_move_exp(a) + _move_exp(b)) % 4 == 0


def _full_move_set(n):
    """求解器使用的完整动作集（与前端 parseMoves 兼容）。"""
    moves = []
    for base in ["U", "D", "F", "B", "L", "R"]:
        moves += [base, base + "'", base + "2"]
    for base in ["TR", "TL", "TU", "TD", "MR", "ML", "MU", "MD"]:
        moves += [base, base + "'", base + "2"]
    if n >= 5:
        for base in ["M", "E", "S"]:
            moves += [base, base + "'", base + "2"]
...
def _home_fraction(cube):
    """贴纸归位率：等于所在面颜色的贴纸占比（已解=1.0，随机≈1/6）。"""
    n = cube.size
    total = 6 * n * n
    home = 0
    for f in range(6):
...
def short_solve(cube, max_depth=2):
    """近已解状态的快速求解：小深度穷举。
    解决"一步/两步打乱却要上百步还原"的问题。
    深度 1-2 用完整动作集；深度 3 用外层动作集（3 步外层打乱 ~0.2s，
    全动作集深度 3 约 16s 不可行）。返回动作列表；
    状态过乱或超深时返回 None（走正常降阶）。"""
    n = cube.size
    if cube.is_solved():
        return []
    if _home_fraction(cube) < 0.5:
        return None
    moveset = _full_move_set(n)
    base = cube.serialize()
    for m in moveset:
        c = BigCube(n, base)
        c.apply(m)
...
def simplify_moves(moves):
    """合并相邻同基底动作到不动点：U U->U2, U U'->删除, U U2->U', X2 X2->删除…"""
    out = list(moves)
    while True:
        changed = False
        nxt = []
        i = 0
        while i < len(out):
            if i + 1 < len(out) and _move_base(out[i]) == _move_base(out[i + 1]):
                e = (_move_exp(out[i]) + _move_exp(out[i + 1])) % 4
                if e == 0:
                    i += 2
                    changed = True
                    continue
                b = _move_base(out[i])
                nxt.append(b if e == 1 else (b + "2" if e == 2 else b + "'"))
                i += 2
                changed = True
                continue
            nxt.append(out[i])
...
def _sort_shortest(lib):
    """把边原语库各 (p,s) 候选按动作长度升序排序（惰性、幂等）。
    让 find_cycle 等"取首个可用"的插入优先使用最短原语。"""
    if lib is not None and not getattr(lib, "_by_pair_sorted", False):
        for lst in lib._by_pair.values():
            lst.sort(key=lambda e: len(e[2]))
        lib._by_pair_sorted = True
    return lib
...
def _best_edge_move(cube, unsolved, library, facet_info, line_of, order_key):
    ...
    for cyc, other, moves in _iter_primitives(library):
        for i in range(3):
            ...
            better = best_delta is None or delta > best_delta
            if not better and delta == best_delta:
                if order_key(sid) < order_key(best_sid):
                    better = True
                elif order_key(sid) == order_key(best_sid) and len(moves) < best_len:
                    better = True
            if better:
