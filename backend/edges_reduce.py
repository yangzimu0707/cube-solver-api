...
def solve_reduction_444(cube, edge_lib=None, center_lib=None):
    ...
    for _ in range(6):
        s3 = extract_3x3(cube)
        try:
            sol = solve_with_timeout(s3)
        except Exception as e:
            msg = str(e)
            is_flip = "Flip" in msg
            is_parity = "Parity" in msg
            if is_flip and not applied_oll:
                cube.apply_moves(OLL_PARITY_444)
                moves_out.extend(OLL_PARITY_444)
                applied_oll = True
                continue
            if is_parity and not applied_pll:
                cube.apply_moves(PLL_PARITY_444)
                moves_out.extend(PLL_PARITY_444)
                applied_pll = True
                continue
            return moves_out, f"kociemba 收尾失败: {e}"
        moves_out.extend(sol.split())
        cube.apply_moves(sol.split())
        return moves_out, None
    return moves_out, "kociemba 收尾超过尝试上限"
